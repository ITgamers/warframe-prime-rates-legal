---
title: Terms of Use — Warframe Prime Rates
permalink: /terms/
---

# Terms of Use

**Warframe Prime Rates** · by steamonlyitgamer

**Effective:** September 12, 2026

By installing or using Warframe Prime Rates ("the app"), you agree to these
terms. If you do not agree, do not use the app.

## 1. What the app is

Warframe Prime Rates is a free tool for Warframe players. It analyses
warframe.market prices, tracks your trades and inventory, and helps plan
builds and purchases.

**The app is independent.** It is not made, approved, endorsed or supported by
Digital Extremes, by warframe.market, or by Overwolf. *Warframe* and related
names and artwork are trademarks of Digital Extremes Ltd.

## 2. Your accounts and other services' rules

The app works alongside services you have your own agreements with, and you
remain bound by them:

- Warframe's End User License Agreement and Terms of Service
- warframe.market's rules and terms
- Overwolf's terms of use

Using third-party tools alongside a game is **at your own risk**. It is your
responsibility to make sure your use of the app complies with these services'
rules.

## 3. Actions on your warframe.market account

If you sign in to warframe.market, the app can create, change and remove
listings on your account. **These are real listings that involve real
platinum.**

The app asks you to confirm before it makes any such change, and it only acts
when you tell it to. You are responsible for every listing and order change
made through your account, including the prices you set.

## 4. Information is not a guarantee

Prices, trends, estimates, trade suggestions and build calculations are
worked out automatically from market and game data. They can be incomplete,
delayed or simply wrong, and markets move.

Nothing in the app is financial advice or a promise of any result. Decisions
about what to buy, sell, craft or build are yours.

## 5. Ads

The app is free and supported by ads served through Overwolf. You can manage
your ad consent choices in the app under Settings → Ad privacy.

## 6. Ownership

The app's source code is made available under the MIT License. That license
does **not** cover the app's logo and icon artwork, which remain the property
of the developer. *Warframe*, its game content and related marks belong to
Digital Extremes.

## 7. Acceptable use

Do not use the app to break the rules of Warframe, warframe.market or
Overwolf, to automate trading or harass other players, or to overload the
services the app relies on.

## 8. No warranty

The app is provided **"as is" and "as available"**, without warranties of any
kind, express or implied, including warranties of merchantability, fitness for
a particular purpose and non-infringement. We do not promise that it will be
error-free, uninterrupted, or compatible with future changes to Warframe,
warframe.market or Overwolf.

## 9. Limitation of liability

To the fullest extent allowed by law, the developer is not liable for any indirect,
incidental or consequential loss, or for any loss of in-game items, platinum,
account access or data, arising from your use of the app or your inability to
use it — including actions taken on your behalf on warframe.market and any
action taken against your accounts by another service.

## 10. Ending use

You can stop using the app at any time by uninstalling it. We may change or
discontinue the app at any time.

## 11. Changes to these terms

If these terms change, the updated version will be posted at this address with
a new effective date. Continuing to use the app after that means you accept
the updated terms.

## 12. Governing law

These terms are governed by the laws of the State of Texas, United States,
without regard to its conflict-of-law rules.

## 13. Contact

Questions about these terms: steamonlyitgamer@gmail.com
